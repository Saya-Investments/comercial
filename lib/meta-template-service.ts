// Helpers para Meta Business API — gestion de plantillas de WhatsApp.
// Adaptado de CRM_Sayasent_Colombia sin la parte de imagenes (Resumable Upload).
// Solo soporta plantillas con header TEXT o sin header.
...
/**
 * Crea una template en Meta Business API (POST /{WABA}/message_templates).
 * Soporta header TEXT, body con variables, footer y botones QUICK_REPLY.
 * NO soporta header con imagen/video/documento (esa feature fue excluida).
 */
export async function createMetaTemplate(input: CreateTemplateInput) {
  ...
  // Header: solo TEXT (IMAGE/VIDEO/DOCUMENT no se soportan en este CRM)
  if (input.header) {
...
/**
 * Lista todas las templates del WABA (hasta 1000).
 * Se usa para sync: refrescar estados en BD segun lo que diga Meta.
 */
...
/**
 * Borra una template en Meta (DELETE por nombre).
 * Meta no permite DELETE por ID, solo por name.
 */
...
/**
 * Consulta el estado de una template especifica en Meta (por nombre).
 */
