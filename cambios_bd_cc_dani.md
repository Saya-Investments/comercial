...
## Cambios para Estrategia Call Center (pendientes de implementar)

Estos cambios soportan la distribucion 30/70 de leads calientes entre call center y asesores. Ver `ESTRATEGIA_CALL_CENTER.md` para contexto completo.

### Nueva tabla: `bd_call_center`

**Proposito:** Registrar los usuarios del call center que gestionan leads calientes.

| Columna | Tipo | Descripcion |
|---------|------|-------------|
| `id_call_center` | UUID (PK) | Identificador unico |
| `nombre` | VARCHAR(100) | Nombre completo |
| `leads_en_cola` | INTEGER (default 0) | Leads actualmente en gestion (max 2) |
| `disponibilidad` | VARCHAR(20) | 'disponible' o 'no disponible' |
| `fecha_creacion` | TIMESTAMPTZ | Fecha de registro |
| `fecha_actualizacion` | TIMESTAMPTZ | Ultima actualizacion |

**Nota:** Tabla aparte de `bd_asesores` porque el call center no participa del modelo de 3 factores ni del enrutamiento de asesores.

### Cambios en tabla existente: `bd_leads`

| Columna nueva | Tipo | Descripcion |
|---------|------|-------------|
| `asignado_call_center` | UUID (FK, nullable) | Si tiene valor, el lead fue asignado primero al call center. FK a `bd_call_center.id_call_center` |

### Cambios en tabla existente: `hist_asignaciones`

| Columna nueva | Tipo | Descripcion |
|---------|------|-------------|
| `via_call_center` | BOOLEAN (default false) | Indica si esta asignacion paso primero por call center antes de llegar al asesor |

### Cambios en tabla existente: `config_modelo`

Nuevos parametros configurables:

| Parametro | Valor inicial | Descripcion |
|-----------|--------------|-------------|
| `porcentaje_call_center` | 0.3 | Probabilidad de que un lead caliente vaya al call center (30%) |
| `score_umbral_caliente` | 0.7 | Score minimo para clasificar un lead como caliente (70 de 100) |
| `capacidad_maxima_call_center` | 2 | Max leads en cola por persona del call center |

### Resumen de relaciones nuevas

```
bd_call_center (nueva)
  └── bd_leads.asignado_call_center (FK)

bd_leads (modificada)
  ├── asignado_call_center -> bd_call_center.id_call_center

hist_asignaciones (modificada)
  ├── via_call_center (boolean)

config_modelo (modificada)
  ├── porcentaje_call_center
  ├── score_umbral_caliente
  └── capacidad_maxima_call_center
```
