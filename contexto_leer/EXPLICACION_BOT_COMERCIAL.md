...
### Seccion 0 — Configuracion (lineas 1-85)
...
**Importante:** La linea 61 indica reemplazar con los tokens reales del numero comercial. Actualmente los tokens ya estan configurados en el codigo.

---

### Seccion 1 — send_whatsapp (lineas 88-107)
...
### Seccion 2 — Instancias de componentes (lineas 110-127)
...
### Seccion 3 — RAG con Elasticsearch (lineas 130-159)
...
### Seccion 4 — Herramientas del Agente (lineas 162-414)

El agente tiene **4 herramientas**. Cada una es una funcion que el agente puede decidir llamar segun el contexto del mensaje.

#### 4.0 — Mapeos de Scoring (en `help_helpers.py`)

Convierten las etiquetas de texto a numeros para el calculo (definidos en `help_helpers.py` y importados en `app.py`):
...
#### 4.1 — `analizar_mensaje_lead` (lineas 174-297)
...
4. **Calcula el scoring** con esta formula (5 variables):

```
raw_score = 0.20 * sentimiento + 0.25 * interes + 0.15 * tiempo + 0.30 * intencion + 0.10 * datos_capturados

score_nuevo = score_anterior * 0.60 + raw_score * 0.40
```

Esto es un **EMA (Exponential Moving Average)**: el score no cambia bruscamente, sino que va acumulando historia. El `alpha=0.40` significa que el 40% del score viene de este mensaje y el 60% del historial. La variable `datos_capturados` mide el porcentaje de datos personales (DNI, nombre, correo) que el lead ha proporcionado.
...
6. **Acciones automaticas**:
   - Si `intencion=baja` + `sentimiento=negativo` + `score < 0.30` (umbral_descarte) → descarta el lead
   - Si `score >= 0.75` (umbral_derivacion) o `derivar=true` → imprime log de derivacion (el agente puede llamar a `derivar_a_asesor`)

7. **Retorna el reply** generado por GPT-4

---

#### 4.2 — `capturar_datos_lead` (lineas 300-355)
...
#### 4.3 — `derivar_a_asesor` (lineas 358-413)
...
### Seccion 5 — Prompt y Agente (lineas 416-445)
...
### Seccion 6 — Forward a Vercel (lineas 448-479)

Copia opcional del webhook hacia una app en Vercel (CRM). Se ejecuta en un hilo separado para no bloquear la respuesta.

---

### Seccion 7 — Webhook Flask (lineas 482-575)

Esta es la ruta principal que recibe los mensajes de Meta.

**GET** `/hello` → Verificacion del webhook (Meta lo llama una vez para confirmar).

**POST** `/hello` → Mensaje entrante. Flujo:
...
### Seccion 8 — Run Local (lineas 578-582)

Arranca Flask en el puerto `8080`.
...
Supongamos un lead con `score_anterior = 0.30` que responde en 1 minuto con: *"Si, quiero cotizar el seguro de vida, cuanto cuesta la cuota mensual?"*. El lead ya proporcionó su nombre y DNI (2 de 3 datos = 0.67).
...
- intencion = **alta** → 1.0
- tiempo = 60 seg → 1.0
- datos_capturados = 2/3 → 0.67

```
raw_score = 0.20*1.0 + 0.25*1.0 + 0.15*1.0 + 0.30*1.0 + 0.10*0.67 = 0.967

score_nuevo = 0.30 * 0.60 + 0.967 * 0.40 = 0.18 + 0.387 = 0.567
```

El score sube de 0.30 a 0.567. Con otro mensaje positivo similar:
```
score_nuevo = 0.567 * 0.60 + 0.967 * 0.40 = 0.340 + 0.387 = 0.727
```

Con un tercer mensaje positivo ya superaria el umbral de 0.75 → se sugiere derivacion a asesor.

---

## Tablas de BD (esquema `comercial`)

### Tablas del Bot (8 tablas)
...
comercial.hist_clientes         ← Historico de leads convertidos a clientes (usado por CRM, no por el bot)
comercial.matching              ← Scores pre-calculados lead-asesor
comercial.config_modelo         ← Pesos y umbrales configurables
```

### Tablas del CRM (7 tablas)

Estas tablas las usa el CRM (app en Vercel) para gestionar el flujo comercial. El bot no las escribe directamente.

```
comercial.crm_usuarios          ← Usuarios del CRM (login, roles: Admin/Manager/Agente)
comercial.crm_tareas            ← Tareas manuales asignadas a asesores sobre leads
comercial.crm_campanas          ← Campanas comerciales masivas
comercial.crm_campana_leads     ← Tabla intermedia campana-lead (many-to-many)
comercial.crm_plantillas        ← Plantillas de mensajes reutilizables
comercial.crm_citas             ← Agenda de citas/reuniones con leads
comercial.crm_notas             ← Notas y observaciones manuales sobre leads
```

Ver detalle de los cambios de schema en `cambios_daniel_13_04_2026.md` (tablas nuevas: `cuotas_semanales`, cambios a `bd_asesores`, `matching.nivel_al_asignar`).
...
[Mensajes]  score sube/baja con cada interaccion
     |
     +-- score < 0.30 + negativo --> estado="descartado"
     |
...
```
comercial/
  app.py                            ← App Flask principal (webhook + agente + tools)
  ...
  comercial_schema.sql              ← DDL completo de la BD (15 tablas)
  prueba_meta.py                    ← Script de prueba para Meta API
  Dockerfile                        ← Dockerfile para despliegue
  requirements.txt                  ← Dependencias Python
  contexto/                         ← Documentacion del proyecto
    CONTEXTO_NEGOCIO.md
    EXPLICACION_BOT_COMERCIAL.md    ← Este archivo
    MODELO_SCORING.md
    cambios_tablas_crm.md           ← Detalle de las 7 tablas CRM nuevas
```
...
|---|---|---|
| `app.py` | `app.py` | App Flask + webhook + agente |
...
Contiene todos los prompts del sistema centralizados. Esto permite modificar el comportamiento del bot sin tocar la logica de `app.py`.
...
| `prompt_sistema_agente()` | `app.py` → seccion 5 | Prompt de sistema del agente LangGraph (define personalidad, reglas, cuando usar herramientas) |
...
Funciones utilitarias y constantes de scoring. Separa la logica de calculo de `app.py` para mantenerlo limpio.
...
| `calcular_datos_capturados(lead)` | Calcula % de datos personales capturados (DNI, nombre, correo): 0/3=0.0, 1/3=0.33, 2/3=0.67, 3/3=1.0 |
| `calcular_raw_score(sentimiento, interes, intencion, tiempo, datos, config)` | Calcula score ponderado: `w_s*S + w_i*I + w_t*T + w_c*C + w_d*D` |
...
| Funcion | Que hace |
|---------|----------|
| `debe_descartar_lead(intencion, sentimiento, score, config)` | `True` si intencion=baja + sentimiento=negativo + score<umbral_descarte (default 0.30) |
...
- Comercial tiene: 11 funciones enfocadas en scoring (incluyendo `calcular_datos_capturados`), decisiones automaticas y formato
...
```
1. WhatsApp envia POST a /hello
2. app.py parsea el mensaje
3. app.py llama a component_firestore.py para log
4. app.py invoca el agente LangGraph
...
10. app.py registra outbound en PostgreSQL y Firestore
11. app.py envia respuesta por WhatsApp
```
